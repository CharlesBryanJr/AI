...
def minimax(board, depth, alpha, beta, maximizingPlayer):
    ...
    if maximizingPlayer:  # max player plays X
        ...
        child_list.sort(key=lambda x: tic_tac_toe_heuristic(x, 'X'), reverse=True)
...
def evaluate_potential_line(line, player_val, opponent_val):
    ...
    potential_win_for_player = player_count > 0 and opponent_count == 0
    potential_win_for_opponent = opponent_count > 0 and player_count == 0
